...
## Plot time-series dataset / Wavelet transform
...
def plot_signal_plus_average(ax, time, signal, average_over = 5):
    ...
    ax.set_title('Signal + Time Average', fontsize=9)
...
fig, ax = plt.subplots(figsize=(10,2))
ax.set_xlabel('Frequency [Hz / year]', fontsize=9)
ax.set_ylabel('Amplitude', fontsize=9)
plot_fft_plus_power(ax, time, signal)
plt.show()

## Plot Continuous Wavelet Transform (Scaleogram)
